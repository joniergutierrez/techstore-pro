...
router.post('/firma', verificarToken, async (req, res) => {
    ...
    console.log("🛒 PRODUCTOS RECIBIDOS EN PAGO:");
...
// Solo crear la orden cuando Wompi confirme el pago
// y todavía no exista una orden asociada.
if (wompiTx.status === 'APPROVED' && !transaccion.orden) {
    ...
    // al formato que necesita el modelo Orden.
    const productosOrden = pod.productos.map(item => ({

        productos: item.producto,

        calidad: item.cantidad

    }));


    // Crear la orden con estado PAGO_CONFIRMADO
    ...
    // Guardar la referencia de la orden
